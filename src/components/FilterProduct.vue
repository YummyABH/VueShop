...
export default {
    ...
    methods: {
        dataRequest(allProducts) {
            const text = event.target.textContent;
            this.$emit('productRequest', text, allProducts)
        },
        dataDiscount() {
            const text = event.target.textContent;
            this.$emit('discountRequest', text)
        }
    },
}
</script>
...
<template>
    <aside class="filter">
        <div class="filter__item _icon _active" @click="dataRequest(true)"><span>Вся продукция</span>
            <svg class="products-icon" width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                <path id="Product" d="M1 1H7V7H1V1ZM1 11H7V17H1V11ZM11 1H17V7H11V1ZM11 11H17V17H11V11Z" stroke="white"
                    stroke-width="2" />
            </svg>
        </div>
        <div class="filter__item _icon" @click="dataDiscount()"><span>Скидки</span>
            <svg class="products-icon" width="18" height="18" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="filter__item" @click="dataRequest(false)">Автомобильные весы</div>
        <div class="filter__item" @click="dataRequest(false)">Сепаратор</div>
        <div class="filter__item" @click="dataRequest(false)">Периферийное оборудование для линий</div>
    </aside>
</template>
...
.filter__item._active {
    ...
    background-color: var(--dived-dafault);
